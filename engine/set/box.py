...
class Box:
    ...
    # constructor
    def __init__(obj, lb = np.matrix([]), ub = np.matrix([])):
        # @lb: lower-bound vector (np.matrix)
        # @ub: upper-bound vector (np.matrix)
    ...
    def affineMap(obj, W, b = np.matrix([])):
        ...
        for i in range(n):
            v = new_generators[i, :].transpose()
            new_lb[i] = new_center[i] - np.linalg.norm(v, np.inf)
            new_ub[i] = new_center[i] + np.linalg.norm(v, np.inf)
