...
class Star:
    ...
    def __init__(obj,
                ...
                C = np.matrix([]),          # constraint matrix
                d = np.matrix([]),          # constraint vector
                pred_lb = np.matrix([]),    # lower bound vector of predicate variable
                pred_ub = np.matrix([]),    # upper bound vector of predicate variable
                state_lb = np.matrix([]),   # lower bound of state variables
                state_ub = np.matrix([]),   # upper bound of state variables
        ...
        if V.size and C.size and d.size:
            ...
            if pred_lb.size and pred_ub.size:
                ...
                obj.predicate_lb = np.matrix([])
                obj.predicate_ub = np.matrix([])
    ...
    # -----first need to implement Star sample but currently there is no Star.contains() function due to no polyhedron
    # sampling a star set
    # def sample(obj, N):
    ...
    #             X = (ub[i] - lb[i])[0,0] * np.random.rand(2*N, 1) + lb[i]
    #             V1 = np.vstack((V1, np.transpose(X))) if V1.size else np.transpose(X)

    #         V = np.array([])
    #         for i in range(2*N):
    #             if obj.contains(V1[:, i]):
    #                 V = np.hstack((V, V1[:, i])) if V.size else V1[:, i]
    ...
    def getMin(obj, index, lp_solver = 'gurobi'):
        ...
        if (f == 0).all():
            ...
            if lp_solver == 'gurobi':
                ...
                min_.Params.LogToConsole = 0
                min_.Params.OptimalityTol = 1e-9
                x = min_.addMVar(shape=obj.dim, lb=obj.predicate_lb, ub=obj.predicate_ub)
                min_.setObjective(f @ x, GRB.MINIMIZE)
                C = sp.csr_matrix(obj.C)
                d = np.array(obj.d).flatten()
                min_.addConstr(C @ x <= d)
    ...
    def getMax(obj, index, lp_solver = 'gurobi'):
        ...
        if (f == 0).all():
            ...
            if lp_solver == 'gurobi':
                ...
                max_.Params.LogToConsole = 0
                max_.Params.OptimalityTol = 1e-9
                x = max_.addMVar(shape=obj.dim, lb=obj.predicate_lb, ub=obj.predicate_ub)
                max_.setObjective(f @ x, GRB.MAXIMIZE)
                C = sp.csr_matrix(obj.C)
                d = np.array(obj.d).flatten()
                max_.addConstr(C @ x <= d)
