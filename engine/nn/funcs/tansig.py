...
class TanSig:
    ...
    def reach_star_approx(I,                                # input star set
        ...
        if method == 'approx-star-no-split' or method == 'approx-star':
            if relaxFactor == 0:
                S = TanSig.multiStepTanSig_NoSplit(I, disp_opt, lp_solver)
            else:
                S = TanSig.relaxedMultiStepTanSig_NoSplit(I, relaxFactor, disp_opt, lp_solver)
...
#------------------check if this function is working--------------------------------------------
    ...
    def reach_star_approx_split(I):
        ...
        for i in range(n):
            ...
            O = np.array([])                                                                # check
            for j in range(m):                  
                O = np.vstack((O, TanSig.stepTanSig_Split(S[j], i)))                     # check
            S = O
        return S
            
 #------------------check if this function is working--------------------------------------------
    def stepTanSig_Split(I, index):
        # @I: input star set
        ...
        if l == u:
            ...
            if isinstance(I.Z, Zono):
                ...
                new_Z = np.array([])
            return Star(new_V, new_C, new_d, new_predicate_lb, new_predicate_ub, new_Z)
    ...
    def multiStepTanSig_NoSplit(I, disp_opt = '', lp_solver = 'gurobi'):
        ...
        # @yl: yl = tansig(l), output of tansig at lower bound
        # @yu: yu = tansig(u), output of tansig at upper bound
...
#------------------check if this function is working--------------------------------------------
    # def relaxedMultiStepTanSig_NoSplit(I, relaxFactor = 0, disp_opt = '', lp_solver = 'gurobi'):
    #     # @I: input star set
    ...
    #     assert isinstance(I, Star), 'error: input is not a star'
    #     assert relaxFactor >= 0 and relaxFactor <= 1, 'error: invalid relax factor, must be between 0 and 1'
    ...
    #     N = I.dim
    #     if disp_opt == 'display':
    #         print('\nComputing (1-%.3f) x %d = %d lower-bounds, i.e. relaxing %2.2f%%: ' % (relaxFactor, len(l), n1, 100*relaxFactor))
    #     l2 = I.getMins(midx(0:n1))
    ...
    def stepLogSig_rstar(I, index, l, u, y_l, y_u, dy_l, dy_u):
        ...
        if l == u:
            ...
            return RStar(new_V, I.C, I.d, D_L, D_U, lb, ub, I.iter)
        else:
            # print('I: \n', I)
            # print('I.V: \n', I.V)
            ...
            return RStar(new_V, new_C, new_d, D_L, D_U, lb, ub, I.iter)
...
#------------------check if this function is working--------------------------------------------
    ...
    def reach_zono_approx_multipleInputs(I, parallel = ''):
        ...
        if parallel == 'parallel':
            for i in range(p):
                S1 = TanSig.reach_zono_approx(I[i])
                S = np.array([S, S1]) if S.size else S1
        else:
# implement parallel multiprocess computation (parfor in matlab)
            for i in range(p):
                S1 = TanSig.reach_zono_approx(I[i])
                S = np.array([S, S1]) if S.size else S1

    #-------------------------------- over-approximate reachability analysis with abstract domain -----------------------------------#
    def stepTanSig_absdom(I, index, l, u, y_l, y_u, dy_l, dy_u):
        # @I: input star set
